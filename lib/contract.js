const Buffer = require('buffer/').Buffer; // They use a weird buffer polyfill that overrides the normal Buffer
...
Contract.prototype.do_execute = helper.asyncCommand(async function(subcmd, opts, args, cb) {
    ...
    if (opts.signing_key) {
        ...
        if (opts.signing_key === '-') {
            signingKey = await fs.readFile(process.stdin.fd, 'utf8');
        } else {
            signingKey = await fs.readFile(opts.signing_keyfile, 'utf8');
        }
    } else {
    ...
    if (opts.payload) {
        ...
        if (opts.payload_file === '-') {
            payload = await fs.readFile(process.stdin.fd, 'utf8');
        } else {
            payload = await fs.readFile(opts.payload_file, 'utf8');
        }
    } else {
